...
// Proteção mínima apenas no erro da URL
$mostrar_modal_sucesso = isset($_GET['success']);
$mensagem_erro = isset($_GET['error']) 
    ? '<div class="card-panel red lighten-4 red-text text-darken-2">' . htmlspecialchars($_GET['error'], ENT_QUOTES, 'UTF-8') . '</div>' 
    : '';
// Busca publicações
$sql = "SELECT 
...
<body>
    ...
    <main class="container">
        ...
        <?php if ($mensagem_erro): ?>
            <div class="row"><div class="col"><?= $mensagem_erro ?></div></div>
        <?php endif; ?>
        <!-- MODAL SUCESSO -->
